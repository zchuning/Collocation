...
# # x: [N x T x dim_x] input
# # returns: scalar loss function of residuals, or scalar sum of squared residuals
# # when `pair_loss_function` is None.
# def cost_function(pair_residual_function, init_residual_function, x,
#                   pair_loss_function = None):
#   dim_x = x.shape[-1]
#   x_curr = x[:,:-1,:]
#   x_next = x[:,+1:,:]
#   # reshape time to batch dimension
#   x_curr = tf.reshape(x_curr, [-1, dim_x])
#   x_next = tf.reshape(x_next, [-1, dim_x])
#   cost = 0.0
#   if pair_residual_function:
#     r_pair = pair_residual_function(x_curr, x_next)
#     if pair_loss_function:
#       cost += tf.reduce_sum(pair_loss_function(r_pair))
#     else:
#       cost += tf.reduce_sum(tf.square(r_pair))
#   if init_residual_function:
#     r_init = init_residual_function(x[:,0,:])
#     cost += tf.reduce_sum(tf.square(r_init))
#   return cost
...
def make_blocks(pair_residual_func, init_residual_func, final_residual_function, x,
                pair_loss_func = None):
  dim_x = x.shape[-1]
  T = x.shape[1]
  ...
  R_final, J_final = jacobian(final_residual_function, x[:,-1,:])
  # t2 = time.time()
  ...
  if M is not None:
    ...
    # TODO This doesn't support goal
    MJ_curr = [None] * T
    MJ_prev = [None] * (T-1)
    MJ_curr[0] = J_init
    for t in range(T-1):
...
# pair_residual_function: maps ([N x dim_x], [N x dim_x]) pair to [N x dim_r]
# init_residual_function: maps [N x dim_x] to [N x dim_r]
# x: [N x T x dim_x] initial guess
# pair_loss_func: function mapping [N x dim_r] to [N x 1] losses
# (when None, L2 loss is used)
# damping: scalar weight of the added main diagonal to ensure positive definiteness
# noise_amount: None or scalar amount to add Gaussian noise with covariance
# defined by the Gauss-Newton matrix (see: Riemannian Langevin dynamics).
# returns: [N x T x dim_x] solution
...
@tf.function
def solve_step_inference(pair_residual_function, init_residual_function, final_residual_function, x,
               pair_loss_func = None, damping=0.0, noise_amount=None):
  # t0 = time.time()
  r, J_curr, J_prev, MJ_curr, MJ_prev = make_blocks(pair_residual_function,
                                  init_residual_function, final_residual_function, x, pair_loss_func)
  # t1 = time.time()
  # print(f"Make_blocks time: {t1 - t0}")
  dx = gauss_newton_solve(r, J_curr, J_prev, MJ_curr, MJ_prev, damping, noise_amount)
  dx = tf.stack(dx, axis=1)
  # if tf.reduce_any(tf.math.is_nan(dx)):
  #   import pdb; pdb.set_trace()
  # if tf.reduce_any(x > 1e5):
  #   import pdb; pdb.set_trace()
  # import pdb; pdb.set_trace()
  return x - dx