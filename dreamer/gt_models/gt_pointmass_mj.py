...
class GtPointmassModel():
  ...
  def forward_model(self, s, a):
    a = a / 30
    s_prime = s + a
    return tfd.Independent(tfd.Normal(s_prime, 0.001), 1)
...
class GtPointmassMj(mujoco_env.MujocoEnv, utils.EzPickle):
  def __init__(self, wall_length=1.5, val=False):
    self.gt_model = GtPointmassModel()
    ...
    mujoco_env.MujocoEnv.__init__(self, curr_dir + '/../envs/pointmass/assets/pointmass_long.xml', 3)
  ...
  def do_simulation(self, ctrl, n_frames):
    ...
    # Use GT model instead of mujoco
    self.gt_model.state = self.data.qpos.flat
    self.gt_model.do_simulation(ctrl, n_frames)
    self.set_state(self.gt_model.state, np.zeros_like(self.gt_model.state))
