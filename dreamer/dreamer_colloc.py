...
class DreamerColloc(Dreamer):
  def visualize_colloc(self, rewards, dyn_loss, img_pred, act_pred, init_feat):
    ...
    plt.legend()
    plt.savefig(self._c.logdir_colloc / 'lr.jpg')
    plt.show()
    ...
    # TODO self._dynamics.imagine already does this
    ...
    imageio.mimsave(self._c.logdir_colloc / "model.gif", model_imgs, fps=10)
...
def colloc_simulate(agent, config, env, save_images=True):
  ...
  for i in range(0, num_iter, config.mpc_steps):
    ...
    if pt == 'colloc_cem':
      ...
      act_pred, img_pred, feat_pred = agent.collocation_gd(obs, save_images)
